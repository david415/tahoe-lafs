...
class MagicFolderTestMixin(MagicFolderCLITestMixin, ShouldFailMixin, ReallyEqualMixin, NonASCIIPathMixin):
    ...
    def test_alice_bob(self):
        ...
        def Bob_write_a_file(result):
            print "Bob writes a file\n"
            self.file_path = abspath_expanduser_unicode(u"../cafe", base=self.bob_magicfolder.uploader._local_path_u)
            fileutil.write(self.file_path, "moo")
            self.magicfolder = self.bob_magicfolder
            self.notify(to_filepath(self.file_path), self.inotify.IN_CLOSE_WRITE)
        d.addCallback(Bob_write_a_file)
        def Bob_wait_for_upload(result):
            print "Bob waits for an upload\n"
            d2 = self.bob_magicfolder.uploader.set_hook('processed')
            return d2
        d.addCallback(Bob_wait_for_upload)
        d.addCallback(Alice_wait_for_upload)
